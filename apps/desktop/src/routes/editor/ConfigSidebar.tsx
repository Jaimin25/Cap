...
const CURSOR_STYLE_OPTIONS = [
  {
    value: "macos",
    label: "macOS",
    description: "Modern macOS cursor shapes.",
  },
  {
    value: "tahoe",
    label: "macOS Tahoe",
    description: "Classic Tahoe-styled cursors.",
  },
  {
    value: "windows",
    label: "Windows",
    description: "Use the captured Windows cursors.",
  },
] satisfies Array<{ value: CursorStyle; label: string; description: string }>;
...
export function ConfigSidebar() {
  ...
  const cursorStyle = () =>
    ((project.cursor as { style?: CursorStyle }).style ??
      "macos") as CursorStyle;
      ...
      >
        ...
        >
          ...
          <Show when={!project.cursor.hide}>
            ...
            <Field name="Cursor Style" icon={<IconLucideWind />}>
              <RadioGroup
                class="flex flex-col gap-2"
                value={cursorStyle()}
                onChange={(value) =>
                  setProject("cursor", "style" as any, value as CursorStyle)
                }
                disabled={!((project.cursor as any).useSvg ?? true)}
              >
